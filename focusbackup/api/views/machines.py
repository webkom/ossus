...
def set_busy_updating(request, id, busy, client_session=None):
    ...
    # Is the current session the current locker?
        ...
        if busy == '0':
            ...
            change_status = False
            machine.log("warning",
                        "%s did not change the current lock, "
                        "why did you try this?" % client_session)
        ...
        if busy == '1':
            ...
            change_status = False
            machine.log("warning",
                        "%s tried to unlock, but the machine "
                        "is already unlocked.." % client_session)
        ...
        if busy == '0':
            ...
            change_status = False
            machine.log("warning",
                        "%s tried to lock, but the machine "
                        "is already locked.." % client_session)

    elif not client_session:
        machine.log("error", "the client did not provide a "
                             "client session, will try my best..")
